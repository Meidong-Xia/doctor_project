function fp8Binary = decimalTofp8_e4m3(decimalValue)
     %  decimalValue = 0;
     bias = 7;
     m = 3;
    % 确保指数在合法范围内,如果大于等于0 11110 111，则溢出
    if decimalValue >= 2^(15-bias)*(1+(2^(m-1)-1)/2^(m-1))
        fp8Binary = ['0',repmat('1',1,3+m),'0'];
        return;
    elseif decimalValue <= -2^(15-bias)*(1+(2^(m-1)-1)/2^(m-1))
        fp8Binary = ['1',repmat('1',1,3+m),'0'];
        return;
    elseif isnan(decimalValue)
        % NaN
        fp8Binary = ['0',repmat('1',1,4+m)]; 
        return;
    end
    ...
    if exponent < 1-bias  % 非规范数
        ...
        for i = flip(0:2^m-1)
            if mod(i, 2) == 1
                if fraction >= (i/2^m+(i+1)/2^m)/2
                    if i == 2^m-1
                        fp8Binary = [signBit, '0001',dec2bin(0,m)] ;
                        break;
                    else
                        fp8Binary = [signBit, '0000',dec2bin(i+1,m)] ;
                        break;
                    end
                end
            else 
                if i == 0
                    if fraction > (i/2^m+(i+1)/2^m)/2
                        ...
                        fp8Binary = [signBit, '0000',dec2bin(i,m)] ;
                        break;
                    end
                else 
                    if fraction > (i/2^m+(i+1)/2^m)/2
        ...
        % 构建fp8二进制表示
        fp8Binary = [signBit, dec2bin(exponent+bias, 4), fraction];
    end
end
