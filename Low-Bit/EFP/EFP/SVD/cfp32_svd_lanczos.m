function [U,A,V]=cfp32_svd_lanczos(mat)
...
for j=1:mini
    if j~=1
        r(:,j)=single(mat'*U(:,j)-beta(j)*V(:,j-1));
        for i=1:j-1
            r(:,j)=single(r(:,j)-(V(:,i)'*r(:,j))*V(:,i));
        end
        alpha(j)=norm(r(:,j));
        V(:,j)=single(r(:,j)/alpha(j));
        P(:,j)=single(mat*V(:,j)-alpha(j)*U(:,j));
        for i=1:j
            P(:,j)=single(P(:,j)-(U(:,i)'*P(:,j))*U(:,i));
        end
        beta(j+1)=norm(P(:,j));
        U(:,j+1)=single(P(:,j)/beta(j+1));
    else
        r(:,1)=single(mat'*U(:,1));
        alpha(j)=norm(r(:,1));
        V(:,j)=single(r(:,1)/alpha(j));
        P(:,j)=single(mat*V(:,j)-alpha(j)*U(:,j));
        P(:,j)=single(P(:,j)-(U(:,j)'*P(:,j))*U(:,j));
        beta(j+1)=norm(P(:,j));
        U(:,j+1)=single(P(:,j)/beta(j+1));
    end
end
for i=1:mini
    B(i,i)=single(alpha(i));
    B(i+1,i)=single(beta(i+1));
end
U=single(U(:,1:mini));
B=single(B(1:mini,:));
if isT==1
