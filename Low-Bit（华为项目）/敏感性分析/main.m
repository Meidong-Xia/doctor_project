...
flg_p = 1; % 预编码方式，1：SVD(单用户)，2：ZF，3: BD(多用户)，4：EZF
...
ber_t = zeros(length(snr_dB),1);
ber_f = zeros(length(snr_dB),1);
idx = zeros(length(snr_dB),1);
total_run_cnt = 0;
% 生成信道
        if flg_c == 1 % 瑞利信道
...
for i=1:length(snr_dB)
    ...
    for j=1:Nf
        ...
        if mod(j,resolution)==0 
            fprintf('当前进度：%G%%\n 耗时：%G小时\n 总块个数：%G\n 有效块个数：%G\n\n', ...
                ((i-1)*Nf+j)/(length(snr_dB)*Nf)*100,toc/3600,(i-1)*Nf+j,total_run_cnt+run_cnt);
        end
        ...
        % 调制
        moduledDataPre = qammod(ldpcEncodeData,M,'InputType','bit');
        moduledDataPre = moduledDataPre.';

        % 测量信号功率、信号功率归一化（单个流）
        sigPower = sum(abs(moduledDataPre).^2,2)/size(moduledDataPre,2);
        moduledData = diag(sqrt(1./sigPower))*moduledDataPre;
        ...
        elseif flg_p == 3 % BD
            ...
            precodedData = eta*V*moduledData;
        elseif flg_p == 4 % EZF
            [eta,F,stop] = f_ezf_precoding(H,P,Nu,Ns,Nr,flg_b,1/snr(i),flg_a); 
            
            if stop
                continue;
            end
            precodedData = F*moduledData;
        end

        % 接收端combiner（64bit计算精度）
        if flg_b~=1 && flg_p~=2 
            if flg_p == 1 % SVD
                % sym = sign(real(U(1,:)));
                [U,Sigma,~] = pre_mysvd_household(H);
                U = U(:,1:Ns);
        ...
        tmp_idx = tmp_idx+norm(receivedDataPlusNoise-moduledDataPre,2)^2/(Nl/log2(M));

        % 解调
        receivedDataPlusNoise = receivedDataPlusNoise.';
        demoduledData = qamdemod(receivedDataPlusNoise,M,'OutputType','bit'); % approxllr
        ...
        % 统计错误块/bit和运行块个数
        error_cnt_bler = error_cnt_bler + sum(any(ldpcDecodeData~=data)); % BLER
        error_cnt_ber = error_cnt_ber + sum(sum(ldpcDecodeData~=data)); % BER
        run_cnt = run_cnt+1;
    end
    % 统计总运行块
    ...
    % 计算MSE
    idx(i) = tmp_idx/run_cnt;

    % 计算bler/ber
    bler(i) = error_cnt_bler/(run_cnt*Ns*Nu);
    ber(i) = error_cnt_ber/(run_cnt*Ns*Nu*Nb);
    ttt = (3*Sigma^2)/(2*(M-1)/snr(i));
    tttt1 = erfc(sqrt(ttt));
    tttt2 = erfc(3*sqrt(ttt));
    ber_t(i) = (sqrt(M)-1)/(sqrt(M)*log2(sqrt(M)))*tttt1+(sqrt(M)-2)/(sqrt(M)*log2(sqrt(M)))*tttt2;
    ber_f(i) = berawgn(10*log10(Sigma^2/log2(M)/(1/snr(i))),'qam',M);
end
