...
function [U,A,Vt]=cfp16_mysvd(mat)
...
lamda(n)=abs(B(n,n));
for j=n-2:-1:0
    [~,~,fenmu]=i_hp_add(lamda(j+1),abs(B(j+1,j+2)));
    [~,~,result0]=i_hp_div(lamda(j+2),fenmu);
    [~,~,lamda(j+1)]=i_hp_mul(abs(B(j+1,j+1)),result0);
%     lamda(j+1)=abs(B(j+1,j+1))*(lamda(j+2)/(lamda(j+1)+abs(B(j+1,j+2))));
end
u=zeros(n,1);
u(1)=abs(B(1,1));
for j=0:n-2
...
while true
    ...
    iter_num=iter_num+1;
    i_u=n-1;
    while((i_u>=1)&&(abs(BI(i_u,i_u+1))<=1e-2))
        i_u=i_u-1;
    end
    if i_u==0
        break
    ...
    i_l=i_u-1;
    if i_l~=0
        while((abs(BI(i_l,i_l+1))>1e-2)&&(i_u>=1))
            i_l=i_l-1;
            if i_l==0
                break
            end
