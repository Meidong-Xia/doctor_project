function [result_fp8Binary, result_config] = EFP_mul_matrix_e(fp8Binary1, config1, fp8Binary2, config2, base, fraction_tables,table)
    % %     生成一个 4x4 复数域矩阵
    ...
    % 初始化结果矩阵
    % [result_fp8Binary, result_config] = arrayfun(@i_EFP_mul,fp8Binary1,config1,fp8Binary2,config2,'UniformOutput',false);
    % 获取矩阵的尺寸
    ...
    for idx = 1:numElements
        ...
        [result_fp8Binary{idx}, result_config{idx}] = i_EFP_mul(fp8Binary1(idx), config1(idx),fp8Binary2,config2,base, fraction_tables,table);
    end
