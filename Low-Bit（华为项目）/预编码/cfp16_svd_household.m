...
% 这个版本为3.0（2024/2/27）更改，更新16bit算子文件FP16（2_27）的使用，对算法细节进行微调，计算有效率约为100%（已做优化调试）
% 64*8矩阵有效计算迭代次数约为13，离群值（即计算有效（相对误差小于0.5）但迭代次数跑满）占比为18.6%，相对误差中位数为0.02298，最大值为0.12
% 这个版本为4.0（2024/3/27）更改,将相位规范。
function [U,A,Vt]=cfp16_svd_household(mat)
...
lamda(n)=abs(B(n,n));
for j=n-2:-1:0
    fenmu=real(i_hp_add(lamda(j+2),abs(B(j+1,j+2))));
    result0=real(i_hp_div(lamda(j+2),fenmu));
    lamda(j+1)=real(i_hp_mul(abs(B(j+1,j+1)),result0));
%     lamda(j+1)=abs(B(j+1,j+1))*(lamda(j+2)/(lamda(j+2)+abs(B(j+1,j+2))));
end
u=zeros(n,1);
u(1)=abs(B(1,1));
for j=0:n-2
    fenmu=real(i_hp_add(u(j+1),abs(B(j+1,j+2))));
    result0=real(i_hp_div(u(j+1),fenmu));
    u(j+2)=real(i_hp_mul(abs(B(j+2,j+2)),result0));
...
BI=real(BI);
% maxit=n*n;
% maxit=i_hp_mul(n,n);% 避免溢出
% maxit=real(maxit);
maxit=39;% 为节省计算时间，64*8矩阵迭代次数设置上限为20（正常值约为12）
[low_bound_sigma,lad,~]=estimation(BI);
max_bound_sigma=max(abs(BI),[],'all');
tl=i_hp_mul(tol,low_bound_sigma);
mu=i_hp_mul(maxit,underflow);
thresh=max(tl,mu);
while true
    if(iter_num>maxit)
        break;
    ...
    iter_num=iter_num+1;
    i_u=n-1;
    while((i_u>=1)&&(abs(BI(i_u,i_u+1))<=1e-3))
        i_u=i_u-1;
    end
    if i_u==0
        break
    ...
    i_l=i_u-1;
    if i_l~=0
        while((abs(BI(i_l,i_l+1))>1e-3)&&(i_u>=1))
            i_l=i_l-1;
            if i_l==0
                break
            end
    ...
    if direction==1
        result0=real(i_hp_div(abs(BI(i_u,i_u+1)),lad(i_u+1)));
        if(result0<=tol)
            BI(i_u,i_u+1)=0;
        end
    end
    %compute shift
    ft=real(i_hp_mul(fudge,tol));
    ftl=real(i_hp_mul(ft,low_bound_sigma));
    result1=real(i_hp_div(ftl,max_bound_sigma));
    if (result1<=epcl)
        shift=0;
    else
        if direction==1
            s=BI(i_u+1,i_u+1);
            r0=real(i_hp_mul(BI(i_u,i_u),BI(i_u,i_u)));
            r1=real(i_hp_mul(BI(i_u-1,i_u),BI(i_u-1,i_u)));
            r2=real(i_hp_add(real(r0),real(r1)));
            r3=real(i_hp_mul(real(BI(i_u+1,i_u+1)),real(BI(i_u+1,i_u+1))));
            r4=real(i_hp_mul(real(BI(i_u,i_u+1)),real(BI(i_u,i_u+1))));
            r5=real(i_hp_add(real(r3),real(r4)));
            r6=real(i_hp_sub(real(r2),real(r5)));
            d=real(i_hp_div(real(r6),real(2)));
%             d=((BI(i_u,i_u)*BI(i_u,i_u)+BI(i_u-1,i_u)*BI(i_u-1,i_u))-(BI(i_u+1,i_u+1)*BI(i_u+1,i_u+1)+BI(i_u,i_u+1)*BI(i_u,i_u+1)))/2;
            r7=real(i_hp_mul(BI(i_u+1,i_u+1),BI(i_u+1,i_u+1)));
            r8=real(i_hp_mul(BI(i_u,i_u+1),BI(i_u,i_u+1)));
            r9=real(i_hp_add(r7,r8));
            r10=real(i_hp_add(r9,d));
            d2=real(i_hp_mul(d,d));
            r11=real(i_hp_mul(BI(i_u,i_u),BI(i_u,i_u)));
            r12=real(i_hp_mul(BI(i_u,i_u+1),BI(i_u,i_u+1)));
            r13=real(i_hp_mul(r11,r12));
            r14=real(i_hp_add(d2,r13));
            r15=real(i_hp_sqrt(r14));
            r16=real(i_hp_mul(sign(d),r15));
            shift=real(i_hp_sub(r10,r16));
%             shift=(BI(i_u+1,i_u+1)*BI(i_u+1,i_u+1)+BI(i_u,i_u+1)*BI(i_u,i_u+1))+d-sign(d)*sqrt(d*d+BI(i_u,i_u)*BI(i_u,i_u)*BI(i_u,i_u+1)*BI(i_u,i_u+1));
            ...
            r0=real(i_hp_mul(BI(i_l+2,i_l+2),BI(i_l+2,i_l+2)));
            r1=real(i_hp_mul(BI(i_l+2,i_l+3),BI(i_l+2,i_l+3)));
            r2=real(i_hp_add(r0,r1));
            r3=real(i_hp_mul(BI(i_l+1,i_l+1),BI(i_l+1,i_l+1)));
            r4=real(i_hp_mul(BI(i_l+1,i_l+2),BI(i_l+1,i_l+2)));
            r5=real(i_hp_add(r3,r4));
            r6=real(i_hp_sub(r2,r5));
            d=real(i_hp_div(r6,2));
%             d=((BI(i_l+2,i_l+2)^2+BI(i_l+2,i_l+3)^2)-(BI(i_l+1,i_l+1)^2+BI(i_l+1,i_l+2)^2))/2;
            r7=real(i_hp_mul(BI(i_l+1,i_l+1),BI(i_l+1,i_l+1)));
            r8=real(i_hp_mul(BI(i_l+1,i_l+2),BI(i_l+1,i_l+2)));
            r9=real(i_hp_add(r7,r8));
            r10=real(i_hp_add(r9,d));
            d2=real(i_hp_mul(d,d));
            r11=real(i_hp_mul(BI(i_l+2,i_l+2),BI(i_l+2,i_l+2)));
            r12=real(i_hp_mul(BI(i_l+1,i_l+2),BI(i_l+1,i_l+2)));
            r13=real(i_hp_mul(r11,r12));
            r14=real(i_hp_add(d2,r13));
            r15=real(i_hp_sqrt(r14));
            r16=real(i_hp_mul(sign(d),r15));
...
%             shift=(BI(i_l+1,i_l+1)^2+BI(i_l+1,i_l+2)^2)+d-sign(d)*sqrt(d^2+BI(i_l+2,i_l+2)^2*BI(i_l+1,i_l+2)^2);
        ...
        shiftdivs2=real(i_hp_div(shift2,s2));
        if(shiftdivs2)<=epcl
            shift=0;
        end
    end
...
% 计算向量范数
[n,~]=size(x);
xtx=real(i_hp_matrix_mul(complex(x'),complex(x)));
nx=real(i_hp_sqrt(xtx));
...
result2=real(i_hp_add(1,t2));
fenmu=real(i_hp_sqrt(result2));
cs=real(i_hp_div(1,fenmu));
% cs=1/sqrt(1+t^2);
...
G112=real(i_hp_mul(G(1,1),G(1,1)));
G212=real(i_hp_mul(G(2,1),G(2,1)));
result3=real(i_hp_add(G112,G212));
alpha=real(i_hp_sqrt(result3));
% alpha=sqrt(G(1,1)^2+G(2,1)^2);
G122=real(i_hp_mul(G(1,2),G(1,2)));
G222=real(i_hp_mul(G(2,2),G(2,2)));
result4=real(i_hp_add(G122,G222));
beta=real(i_hp_sqrt(result4));
% beta=sqrt(G(1,2)^2+G(2,2)^2);
c1=real(i_hp_div(G(1,1),alpha));
% c1=G(1,1)/alpha;
c2=real(i_hp_div(G(2,2),beta));
% c2=G(2,2)/beta;
s1=real(i_hp_div(G(2,1),alpha));
% s1=G(2,1)/alpha;
s2=real(i_hp_div(G(1,2),beta));
% s2=G(1,2)/beta;
G=real(i_hp_matrix_mul(complex([c1,s1;s2,c2]),complex(G)));
% G=[c1,s1;s2,c2]*G;
...
n=i_max+1;
tri=real(tri);
r0=real(i_hp_mul(tri(i_min+1,i_min+1),tri(i_min+1,i_min+1)));
x=real(i_hp_sub(r0,shift));
...
for i=n-1:-1:i_min+1
    ...
    if i~=n-1
        tri(i+1,i+2)=real(i_hp_mul(-olds,r));
%         tri(i+1,i+2)=-olds*r;
    end
    x=real(i_hp_mul(oldc,r));
%     x=oldc*r;
    y=real(i_hp_mul(-tri(i,i),s));
%     y=-tri(i,i)*s;
    h=real(i_hp_mul(tri(i,i),c));
%     h=tri(i,i)*c;
    [c,s,r]=csr(x,y);
    oldc=c;
    olds=s;
...
tri(i_min+1,i_min+2)=real(i_hp_mul(-h,s));
% tri(i_min+1,i_min+2)=-h*s;
tri(i_min+1,i_min+1)=real(i_hp_mul(h,c));
% tri(i_min+1,i_min+1)=h*c;
end

function [tri,U]=QR_Wilkinson_shift_Iteration_once_upward(tri,U,i_min,i_max,shift)
n=i_max+1;
ct=real(i_hp_sub(i_min,shift));
% ct=i_min-shift;
x=real(i_hp_mul(tri(n,n),tri(n,n)));
% x=tri(n,n)^2;
y=real(i_hp_mul(tri(n-1,n-1),tri(n-1,n)));
...
for k=n-1:-1:i_min+1
    ...
    A=real(i_hp_matrix_mul(complex([c,-s;s,c]),complex([tri(k+1,k+1),0;tri(k,k+1),tri(k,k)])));
    x=A(1,1);y=A(1,2);tri(k,k+1)=A(2,1);tri(k,k)=A(2,2);
    [U(:,k+1),U(:,k)]=updatecsvv(c,s,U(:,k+1),U(:,k));
    [c,s,r]=csr(x,y);
    tri(k+1,k+1)=r;
    if k~=ct+1
        A=real(i_hp_matrix_mul(complex([tri(k,k+1),tri(k,k);0,tri(k-1,k)]),complex([c,s;-s,c])));
        x=A(1,1);tri(k,k)=A(1,2);y=A(2,1);tri(k-1,k)=A(2,2);
    else
        A=real(i_hp_matrix_mul(complex([tri(ct+1,ct+2),tri(ct+1,ct+1)]),complex([c,s;-s,c])));
...
for i=i_min:n-1
    ...
    if i~=i_min
        tri(i,i+1)=real(i_hp_mul(-olds,r));
%         tri(i,i+1)=-olds*r;
    end
    x=real(i_hp_mul(oldc,r));
%     x=oldc*r;
    y=real(i_hp_mul(-tri(i+2,i+2),s));
%     y=-tri(i+2,i+2)*s;
    h=real(i_hp_mul(tri(i+2,i+2),c));
%     h=tri(i+2,i+2)*c;
    [c,s,r]=csr(x,y);
...
tri(n-1,n)=real(i_hp_mul(-h,s));
% tri(n-1,n)=-h*s;
tri(n,n)=real(i_hp_mul(h,c));
% tri(n,n)=h*c;
end
