    ...
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < k; j++) {
            ...
            for (int l = 0; l < n; l++) {
                ...
                sum_real = sum_real+product_real;//扩位相加
                sum_imag = sum_imag+product_imag;
                // Store intermediate results
            }
            output_matrix_product_real[i + m * j] = static_cast<double>(static_cast<half_float::half>(sum_real));
            output_matrix_product_imag[i + m * j] = static_cast<double>(static_cast<half_float::half>(sum_imag));
            }
        }
    } else {
        // 如果有三个输出参数，则创建三个输出矩阵
